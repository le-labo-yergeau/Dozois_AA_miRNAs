...
boxsig <- ggplot(combined_ASVs_sig, aes(x = Treatment, y = Relative_abundance * 100)) +
  ...
  theme_light() +
  facet_nested_wrap(~N_Source + Taxonomy,
                    scales = "free_y",
                    nrow = 1,
                    nest_line = TRUE) +
  theme(
  ...
  labs(x = "", y = "Relative abundance of taxa (%)")
...
# Save base plot
ggsave(boxsig_2, filename = here("Output", "ASVs_sig.tiff"),
       width = 30, height = 12, units = "cm", compression = "lzw")
...
box_sig2
ggsave(box_sig2, file=here("Output","ASVs_sig_stat.tiff"),units="cm", width=30, height=12, compression='lzw')
