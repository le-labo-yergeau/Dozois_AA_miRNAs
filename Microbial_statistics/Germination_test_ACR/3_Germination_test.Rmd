...
boxArabidopsis_fresh <- ggplot(Arabidopsis_fresh, aes(x = Isolate, y = Fresh_weight)) +
  ...
  scale_fill_manual(
    values = c("#277da1", "#f8961e", "#f94144", "#1c3144"),
    guide = guide_legend(label.theme = element_text(face = "italic"))
  ) + scale_colour_manual(
    values = c("#277da1", "#f8961e", "#f94144", "#1c3144"),
    guide = guide_legend(label.theme = element_text(face = "italic"))
  ) + geom_pwc(
    aes(group = Isolate), tip.length = 0.01, method = "wilcox_test",
    label = "{p.adj.signif}", p.adjust.method = "BH", y.position = c(0.9, 1.14, 1.36), hide.ns = TRUE, label.size = 5, size = 0.5)
...
saveRDS(combi3, file=here('Output','Figures', 'Fig_Arabi_isolates.rds'), compression="bzip2")
